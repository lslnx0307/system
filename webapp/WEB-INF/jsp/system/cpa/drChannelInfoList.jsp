...
<body>
	...
	data-options="singleSelect:true,
	    ...
	    <tr>
	    	...
	    	<th data-options="field:'channelTypeName'" width="9%">渠道大类</th>
	        ...
	        <th data-options="field:'status'" width="5%" formatter="formatStatus">渠道状态</th>
	       	...
	       	<th data-options="field:'roi'" width="5%">ROI</th>
	...
	<div id="drChannelInfoTools" style="padding:5px;height:750">
	  	<form id="drChannelInfoForm">
         	 ...
         	 渠道大类:<select id="searchchannelTypeId" name="channelTypeId" class="easyui-combobox" style="width:100px">
	  	        	<option value=''>全部</option>
	...
	<div id="addDrChannelInfoDialog" class="easyui-dialog" title="渠道添加"
		data-options="iconCls:'icon-edit',closed:true,modal:true,minimizable:false,resizable:true,maximizable:false,buttons:'#addDrChannelInfoBtn'" style="width:400px;height:250px;padding:5px;">
		<form id="addDrChannelInfoForm">
			<table align="center">
				<tr>
					...
					<td>
						<select id="addchannelTypeId" name="channelTypeId" class="easyui-combobox" style="width:150px;" data-options="required:true">
		  	           	<c:forEach items="${nameType}" var="map">
							<option value='${map.id }'>${map.name}</option>
						</c:forEach>
		  	           </select>
	...
	<div id="updateDrChannelInfoDialog" class="easyui-dialog" title="渠道修改"
		data-options="iconCls:'icon-edit',closed:true,modal:true,minimizable:false,resizable:true,maximizable:false,buttons:'#updateDrChannelInfoBtn'" style="width:400px;height:250px;padding:5px;">
		<form id="updateDrChannelInfoForm">
			...
			<table align="center">
				<tr>
					...
					<td>
						<select id="updatechannelTypeId" name="channelTypeId" class="easyui-combobox" style="width:150px;" data-options="required:true">
		  	           	<c:forEach items="${nameType}" var="map">
							<option value='${map.id }'>${map.name}</option>
						</c:forEach>
		  	           </select>
...
<script type="text/javascript">
	...
	$('#searchDrChannelInfo').click(function(){
 		$('#drChannelInfoList').datagrid({
			queryParams: {
				...
				channelTypeId: $('#searchchannelTypeId').combobox('getValue'),
	...
	function addDrChannelInfo(){  
		...
		if($('#addchannelTypeId').combobox("getValue")=="" || $('#adddrChannelInfoCode').textbox("getValue").trim()=="" || $('#adddrChannelInfoName').textbox("getValue").trim()==""){
		...
		$.ajax({
			...
			success:function(result){
				if(result.success){
					...
					$("#updatechannelTypeId").combobox('setValue',result.map.drChannelInfo.channelTypeId);
	...
	function updateDrChannelInfo(){  
		...
		if($('#updatechannelTypeId').combobox("getValue")=="" || $('#drChannelInfoCode').textbox("getValue").trim()=="" || $('#drChannelInfoName').textbox("getValue").trim()==""){
			$.messager.alert("提示信息","渠道信息不能为空");
			return false;
		}
   		$.ajax({
