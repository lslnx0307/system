				...
				<tr>
					...
					<td>
						<select id="updateType" name="type" style="width:100px;" class="easyui-combobox" data-options="required:true">
							...
							<option value='7' <c:if test="${drCouponsIssuedRules.type==7}"> selected="selected" </c:if>>开通存管</option>
							<option value='8' <c:if test="${drCouponsIssuedRules.type==8}"> selected="selected" </c:if>>待流失唤回</option>
							<option value='9' <c:if test="${drCouponsIssuedRules.type==9}"> selected="selected" </c:if>>已流失唤回</option>
						</select>
					</td>
					<td>活动名称:</td>
					<td>
