	...
	<form id="updateActivityForm" >
	    	<table align="center">
				...
				<tr>
					...
					<td><input id="updateCode" type="text" name="code" class="easyui-textbox" value="${drActivityParameter.code}" readonly="readonly"/></td>
					<td style="width:100px">层级分类:</td>
					<td>
						<select name="ahsId" style="width: 170px" class="easyui-combobox" id="three_flag" data-options="required:true">
				...
				<tr>
					<td>红包来源:</td>
					<td><input type="text" id="packetSource" name="packetSource" class="easyui-textbox" value="${drActivityParameter.packetSource}" readonly="readonly"/></td>
				</tr>
				<tr>
					<td>还款方式：</td>
					<td colspan="3">
						<input id="repayType1" name="repayType" type="checkbox" value="1" disabled="disabled"/>到期一次还本付息 &nbsp;&nbsp;&nbsp;&nbsp;
						<input id="repayType2" name="repayType" type="checkbox"  value="3,4" disabled="disabled"/>等本等息还款&nbsp;&nbsp;&nbsp;&nbsp;
					</td>
				</tr>
				<tr>
					<td>适用场景：</td>
				...
				<tr style="width:400px;hight:400px">
					...
					<td colspan="3" rowspan="4">
						<textarea id="packetRemak"style="width:300px;height:40px;" name="packetRemak" type="text" value="" readonly="readonly"></textarea>
					</td>
				</tr>
			</table>
		</form>
...
<script type="text/javascript">
	$(document).ready(function(){
		getThreeFlag();
		$('#packetRemak').val("${drActivityParameter.packetRemak}");
	});
	//关闭窗口
	...
	//查询第三层分类
	var ahsId=${drActivityParameter.ahsId}
		   ...
		   onLoadSuccess:function(){
	            ...
	            if (data.length > 0) {
		            $.each(data, function (n, v) {
	            		if(v.id ==ahsId){
	            			 $('#three_flag').combobox('select', ahsId);
	            		}
              		});
	...
	var repayTypes = '${drActivityParameter.repayType}'.split(',');
	for(var i=0;i<repayTypes.length;i++){
		if(repayTypes[i]=='1'){
			$('#repayType1').prop("checked",true);
		}else if(repayTypes[i]=='3' || repayTypes[i]=='4'){
			$('#repayType2').prop("checked",true);
		}else{
			$('#repayType1').prop("checked",true);
			$('#repayType2').prop("checked",true);
		}
