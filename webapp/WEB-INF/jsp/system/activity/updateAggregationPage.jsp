...
<form id="addArticleForm" enctype="multipart/form-data">
	...
	<table >
		...
		<tr>
			...
			<td>
			<input id="arr_startTime" name="arrStartTime" value="${jsActivityAggregationPage.arrStartTimeStr}" style="width:150px;" class="easyui-datetimebox"  data-options="required:true,showSeconds:true" editable="false" missingMessage="该项必须填写" />
			</td>
			<td align="left">
					下架时间:
			</td>
			<td>
				<input id="arr_endTime" name="arrEndTime" value="${jsActivityAggregationPage.arrEndTimeStr}" style="width:150px;" class="easyui-datetimebox"  data-options="required:true,showSeconds:true" editable="false" missingMessage="该项必须填写"/>
			</td>
		</tr>
	</table>
		</div>
	...
	function PreviewImage(articleAddPicFile) {
		...
		function CompareTime(start,end){
	      ...
	      if(allStartDate.getTime() >= allEndDate.getTime()) {
	    	  $.messager.alert('错误提示','结束时间不能小于开始时间','info');  
	    	 	 return false;
	         }else{
	        	 return true;
	         }
