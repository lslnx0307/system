				...
				<tr>
    				...
    				<td>
    					<select id="addContainers" name="containers" style="width:150px;" class="easyui-combobox" data-options="required:true" disabled="disabled">
							<option value='1' <c:if test="${sar.containers==1}">selected="selected"</c:if>>IOS 主包</option>
							<option value='3' <c:if test="${sar.containers==3}">selected="selected"</c:if>>IOS pro版</option>
							<option value='4' <c:if test="${sar.containers==4}">selected="selected"</c:if>>IOS 精英版</option>
