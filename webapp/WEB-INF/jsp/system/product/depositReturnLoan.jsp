...
<body>
	<table id="drProductLoanList" title="放回款管理 <span style='color: #0015FF;'>放款总额</span>：<span id='loanAmountSum' style='color: red;'></span>"  style="height:99%;width:99.9%">
		<thead>
	    <tr>
	...
	function searchDrProductLoan(){
 		$('#drProductLoanList').datagrid({
			...
			onBeforeLoad: function (d) {
			    $.ajax({
				url:"${apppath}/product/depositReturnLoanListSum.do",
				type:"POST",
				data:{
					loanName:$('#searchDrProductLoanloanName').val(),
					refundStartDate:$('#searchDrProductLoanloanstartDate').datebox('getValue'),
				...
				success:function(result){
					$("#loanAmountSum").text(fmoney(result.amount,2));
				}
			});
		} 
		});
	...
	function formatOper(val,row,index){
		if((row.status == 8 ||row.status == 9 ) &&  row.loanStatus==1 && menuType=="1"){
			if(row.repayTypeName == "到期一次还本付息"){
				...
				return	'<a href="#" class="btn l-btn l-btn-small" onclick="toInstallmentRepaymentInfo('+index+')">等息回款</a>'+"   "+
	            '<a href="#" class="btn l-btn l-btn-small" onclick="companyFuiouBalance('+index+')">查看企业账户余额</a>';
			}
		}
...
//跳转到产品分期展示页面
function toInstallmentRepaymentInfo(index){
	$('#drProductLoanList').datagrid('selectRow',index);// 关键在这里 
    var row = $('#drProductLoanList').datagrid('getSelected'); 
	var mainTab = parent.$('#main-center');
	var detailTab = {
			title : "分期产品显示",
			content:"<iframe scrolling='yes' width='100%' height='100%' frameborder='0' src='${apppath}/product/toInstallmentRepaymentInfo.do?id="+row.id+"'></iframe>",
			closable : true
	};
	mainTab.tabs("add",detailTab);
}
