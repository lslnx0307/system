		...
		<div title="产品信息" class="easyui-panel" style="width:100%;height:auto;padding:10px;margin-bottom: 10px">
			<table id="updateProductInfo" align="center" cellspacing="1" cellpadding="1" style="width:auto;">
				...
				<tr>
					...
					<td colspan="2">
						<select id="updateProductType" name="type" style="width: 172px" class="easyui-combobox" data-options="    
							...
							<c:forEach items="${type}" var="map">
								<c:if test="${map.key == 1 || map.key == 2 || map.key == 3 || map.key == 6}">
					...
					<td>会员等级：</td>
					<td><select id="updatemgid" name="mgid" style="width: 172px" class="easyui-combobox">
							<option value=''>请选择</option>
							<c:forEach items="${grade}" var="map">
									<option value='${map.id}' <c:if test="${drProductInfo.mgid== map.id}"> selected="selected" </c:if>>${map.level}</option>
							</c:forEach>
						</select>	
						</td>
				...
				<tr>
					...
					<td colspan="2">
						<input id="productAmount" name="amount" value="${drProductInfo.amount}" type="text" class="easyui-numberbox" 
					...
					<td align="left">产品标识：</td>
					<td>
					<select id="productMarking" name="productMarking" style="width: 172px" class="easyui-combobox">
						<option value=''>请选择</option>
						<c:forEach items="${productMarking}" var="map">
							<option value='${map.key }' <c:if test="${drProductInfo.productMarking == map.key}"> selected="selected" </c:if>>${map.value}</option>
						</c:forEach>
					</select>
				</td>
				</tr>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("#sidUpdate").combobox({    
		    ...
		    $('.isShowPicInfoUpdate').remove();
			    $.ajax({
					...
					success:function(result){
						if(result != ''){
						if(result.drClaimsLoan !=null){
							$("#productAmount").numberbox('setValue',result.drClaimsLoan.loanAmount);
							$("#productDeadline").numberbox('setValue',result.drClaimsLoan.loanDeadline);
						}else{
							$("#productAmount").numberbox('reset');
							$("#productDeadline").numberbox('reset');
						}
 	...
 	function updateDrProductInfoBtn(){
		...
		if($("#updateProductType").combobox('getValue') ==6 && $("#updatemgid").combobox('getValue')==''){
			$.messager.alert("提示信息","V标专属需选择会员等级");
			return false;
		}
		
		if($("#updateProductType").combobox('getValue') !=6 && $("#updatemgid").combobox('getValue')!=''){
			$.messager.alert("提示信息","只有V标专属需选择会员等级");
			return false;
		}
	...
	function updProductPic(){
		var tr = "<tr><td>"+
			...
			"</td>" +
			"<td>"+
			"<input type='radio' name=\"drProductPic["+k+"].type\" value='0' checked='true' style='width:30px;'>企业资料</input>"+
			"<input type='radio' name=\"drProductPic["+k+"].type\" value='1' style='width:30px;'>贷款合同</input>"+
			"</td>"+
			"<td>" +
