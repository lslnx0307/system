...
<input name="oldCode" type="hidden" value="${sysBanner.code}" />
	<table>
		<tr>
			<td>
				标题:
		...
		<tr>
			<td>
				开始时间：
				<input id="startDate" name="startDate" value="${sysBanner.startDateStr}" class="easyui-datetimebox" data-options="required:true"/>
				结束时间：
				<input id="endDate" name="endDate" value="${sysBanner.endDateStr}" class="easyui-datetimebox" data-options="required:true"/>
			</td>
		</tr>
		<tr>	
			<td>
...
<script type="text/javascript">
	...
	function updateSysBanner(){
		...
		if($("#endDate").combobox('getValue') < $("#startDate").combobox('getValue')){
			$.messager.alert("提示信息","结束时间不能在开始时间之前！");
			return false;
		}
