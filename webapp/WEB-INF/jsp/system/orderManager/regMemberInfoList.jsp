...
<body>
	...
	data-options="singleSelect:true,
	    ...
	    <tr>
	        ...
	        <th data-options="field:'level'" width="8%" >会员等级</th>
	        <th data-options="field:'wealth'" width="8%" >财富值</th>
	        <th data-options="field:'isMember'" width="8%">是否点亮</th>
	        ...
	        <th data-options="field:'addTime'" width="12%">绑卡时间</th>
	...
	<div id="drProductInvestTools" style="padding:5px;height:750">
	  	<form id="drProductInvestForm">
			...
			是否投资:<select id="isInvest" class="easyui-combobox" name="isInvest" style="width:80px;">   
					    <option value="">全部</option>   
					    <option value="1">是</option>   
					    <option value="0">否</option>   
					</select> 
			是否充值:<select id="isCrush" class="easyui-combobox" name="isCrush" style="width:80px;">   
					    <option value="">全部</option>   
					    <option value="1">是</option>   
					    <option value="0">否</option>   
					</select> 
			绑卡时间:<input id="startAddBankDate" name="startAddBankDate" class="easyui-datebox" style="width:100px"/>到
 				<input id="endAddBankDate" name="endAddBankDate" class="easyui-datebox" style="width:100px"/>
			是否绑卡:<select id="isBank" class="easyui-combobox" name="isBank" style="width:80px;">   
					    <option value="">全部</option>   
					    <option value="0">已绑卡</option>   
					    <option value="1">未绑卡</option>   
					</select> 
...
<script type="text/javascript">
	...
	//是否点亮操作
	function formatIsMember(val,row,index){
		if(row.isMember==1){
			return "已点亮";
		}else if(row.isMember ==2){
			return "未点亮";
		}
	}
		...
		var cids = $('#cid').combogrid('getValues')+"";
 		$('#regMemberInfoList').datagrid({
			...
			queryParams: {
				...
				startAddBankDate:$('#startAddBankDate').combobox('getValue'),
				endAddBankDate:$('#endAddBankDate').combobox('getValue'),
				isBank:$('#isBank').combo("getValue")+"",
				cids:cids
			}
		}); 
		 //调用ajax方法
