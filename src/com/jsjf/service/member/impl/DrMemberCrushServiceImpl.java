...
public class DrMemberCrushServiceImpl implements DrMemberCrushService {
	...
	@Autowired
	private JsMemberTaskDAO jsMemberTaskDAO;
	@Autowired
	private JsTaskDetailsDAO jsTaskDetailsDAO;
	...
	public void updatePayResult(String paynum) throws Exception{
		log.info(Utils.format(new Date(), "yyyy-MM-dd HH:mm:ss")+":开始实时查询充值结果:"+paynum);
	...
	public void updatePayResult() throws Exception {
		...
		log.info(Utils.format(new Date(), "yyyy-MM-dd HH:mm:ss")+":开始实时查询充值结果");
        ...
        log.info("结束实时查询充值结果 耗时: " + c.get(Calendar.MINUTE) + "分 "  
	...
	public void depositsRecharge(JSONObject json,Integer type,SysFuiouNoticeLog log) throws Exception {
		...
		if(JZHService.verifySignAsynNotice(type==5?new RechargeRspData(message):
			...
			System.out.println("["+Utils.format(new Date(), "yyyy-MM-dd HH:mm:ss")+"]充值验签成功---------------------");
			...
			try {
				if(Utils.isObjectNotEmpty(member)){
					...
					lockFlag = redisClientTemplate.tryLock("fuiouRecharge.id."+member.getUid(), 3, TimeUnit.SECONDS,false);
					if(lockFlag){
								...
								if("0000".equals((String)message.get("resp_code")) ){
									if(drMemberCrush.getUid().intValue() == member.getUid().intValue()){
										...
										DrMemberFundsLog drMemberFundsLog = new DrMemberFundsLog(drMemberCrush.getUid(),record.getId(),drMemberCrush.getAmount(),44,1,
												"充值金额：【"+ drMemberCrush.getAmount().setScale(2)  + "】");
										drMemberFundsLogDAO.insertDrMemberFundsLog(drMemberFundsLog); //充值成功
										// 充值成功 发送站内信
										DrMemberFunds f = new DrMemberFunds();
										f.setBalance(funds.getFuiou_balance());
										...
										JsCompanyAccountLog companyAccountLog=new JsCompanyAccountLog();
										companyAccountLog.setCompanyfunds(59);//资金类型 个人充值
										companyAccountLog.setType(0);//支出
										companyAccountLog.setAmount(poundage);//金额
...
*/										companyAccountLog.setStatus(3);//成功
								...
								Integer count = drMemberCrushDAO.getCrushCount(member.getUid());
								if(count == 1){
									//添加任务完成记录
									param.put("name", "首次充值任意金额");
									List<JsMemberTask> list = jsMemberTaskDAO.selectjsmembTasksList(param);
									Integer taskId = list.size()>0?list.get(0).getId():null;
									JsTaskDetails taskDetails = new JsTaskDetails();
									taskDetails.setMid(member.getUid());
									...
									param.put("taskName", "首次充值任意金额");
									redisClientTemplate.lpush("regAndVerifySendRedUidList".getBytes(),SerializeUtil.serialize(param));
								}
							}
						}
					}
				...
				if(lockFlag){
					redisClientTemplate.del("fuiouRecharge.id."+member.getUid());
				}
			}
		}else{
			//企业网银充值
			...
			if("0000".equals((String)message.get("resp_code"))) {
				...
				if(drProductInfo.getRepayType() == 2 || drProductInfo.getRepayType() == 3 || drProductInfo.getRepayType() == 4){//等本等息，按月付息产品
	    			...
	    			m.put("id", log.getPdId());//期数明细的ID
    				List<DrProductInfoRepayDetail> li = drProductInfoRepayDetailDAO.getrepayDetailById(m);

    				m.clear();
    				m.put("id", log.getPdId());//期数明细的ID
    				m.put("loanStatus", 2);//已回款
    				drProductInfoRepayDetailDAO.updateLoanStatusById(m);
    				
    				m.clear();
    				m.put("pid", drProductInfo.getId());
    				List<DrProductInfoRepayDetail> list = drProductInfoRepayDetailDAO.getrepayDetailById(map);
    								
	    			if(list.size()==0){//已还清
	    				drProductInfoService.updateReFundDrProductLoanStatus(drProductInfo.getId());//已还清
	    			}	
...
*/				jsCompanyAccountLogDAO.insertCompanyAccountLog(companyAccountLog);
			...
			System.out.println("["+Utils.format(new Date(), "yyyy-MM-dd HH:mm:ss")+"]"+(type==5?"pc":type==6?"网银":"app")+"充值验签失败---------------------");
		}
	}
		...
		try {
			...
			if(Utils.isObjectNotEmpty(crush) && (crush.getType() ==5 || crush.getType()==6) && crush.getStatus() !=1){//订单不是成功的
				...
				if(result.isSuccess() && result.getMsg().contains("0000|成功")){
					...
					DrMemberFundsLog drMemberFundsLog = new DrMemberFundsLog(crush.getUid(),record.getId(),crush.getAmount(),44,1,
							"充值金额：【"+ crush.getAmount().setScale(2)  + "】");
					drMemberFundsLogDAO.insertDrMemberFundsLog(drMemberFundsLog);
					// 充值成功 发送站内信
					DrMemberFunds f = new DrMemberFunds();
					...
					JsCompanyAccountLog companyAccountLog=new JsCompanyAccountLog();
					companyAccountLog.setCompanyfunds(44);//资金类型
					companyAccountLog.setType(0);//支出
					companyAccountLog.setAmount(poundage);//金额
					companyAccountLog.setStatus(3);//成功
					companyAccountLog.setRemark(dm.getMobilephone()+"用户充值手续费");
