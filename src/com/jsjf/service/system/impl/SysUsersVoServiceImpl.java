...
public class SysUsersVoServiceImpl implements SysUsersVoService {
	...
	public Map<String, Object> insertUser(SysUsersVo user)throws Exception {
		...
		if (Utils.isObjectEmpty(user.getMobile()) || user.getMobile().length() != 11) {
			map.put("message", "手机号不正确");
			return map;
		}
		if (Utils.isObjectEmpty(user.getLoginId()) || user.getLoginId().length()<2) {
			map.put("message", "登录Id不能为空 或不能少于2个字符");
			return map;
		}
				
		String  password = user.getLoginId().substring(0,2)+user.getMobile().substring(7);
		
		user.setPassword(SecurityUtils.MD5AndSHA256(password));
