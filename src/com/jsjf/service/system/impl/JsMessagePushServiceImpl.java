...
public class JsMessagePushServiceImpl implements JsMessagePushService {
	...
	public BaseResult executePush(JsMessagePush obj) {
		...
		for(JsPushApp app:jsPushAppList){
			...
			try {
				...
				fail++;
				System.out.println("-----push失败:[pushId="+obj.getId()+",appId="+app.getId()+",appName="+app.getAppName());
				e.printStackTrace();
			}
		...
		System.out.println("循环推送总时间"+(l2-l1));
		String resultMsg = "";
		if(success>0){
			resultMsg = "执行成功";
	...
	public BaseResult selectPush(JsMessagePush jsMessagePush){
		...
		System.out.println("总耗时时间"+(System.currentTimeMillis()-l1));
		return result;
	}
