...
public class DrClaimsInfoServiceImpl implements DrClaimsInfoService {
	...
	public void insertDrClaimsInfo(DrClaimsLoan drClaimsLoan, MultipartFile[] claimsFiles) throws Exception {
		...
		if(Utils.isObjectNotEmpty(drClaimsLoan.getServiceCharge())){
			drClaimsLoan.setServiceCharge(drClaimsLoan.getServiceCharge().multiply(new BigDecimal(10000)));
		}
		...
		if(!Utils.strIsNull(drClaimsCustomer.getCompanyName())){
			if(Utils.isObjectNotEmpty(drClaimsCustomer.getRegisterMoney())){
				drClaimsCustomer.setRegisterMoney(drClaimsCustomer.getRegisterMoney().multiply(new BigDecimal(10000)));
			}
		...
		if(drClaimsGuarantee!=null){
			for (DrClaimsGuarantee guarantee : drClaimsGuarantee) {
				if(guarantee.getAssessAmount() != null || guarantee.getGuaranteeAmount() != null || 
					...
					if(Utils.isObjectNotEmpty(guarantee.getSingleGuaranteeAmount())){
						guarantee.setSingleGuaranteeAmount(guarantee.getSingleGuaranteeAmount().multiply(new BigDecimal(10000)));
					}
					if(Utils.isObjectNotEmpty(guarantee.getCompanyGuaranteeAmount())){
						guarantee.setCompanyGuaranteeAmount(guarantee.getCompanyGuaranteeAmount().multiply(new BigDecimal(10000)));
					}
					if(Utils.isObjectNotEmpty(guarantee.getGuaranteeAmount())){
						guarantee.setGuaranteeAmount(guarantee.getGuaranteeAmount().multiply(new BigDecimal(10000)));
					}
		...
		if(drClaimsShareholder!=null){
			for (DrClaimsShareholder shareholder : drClaimsShareholder) {
				if(Utils.isObjectNotEmpty(shareholder.getAmountPaidIn())){
					shareholder.setAmountPaidIn(shareholder.getAmountPaidIn().multiply(new BigDecimal(10000)));
				}
				if(Utils.isObjectNotEmpty(shareholder.getContributionLines())){
					shareholder.setContributionLines(shareholder.getContributionLines().multiply(new BigDecimal(10000)));
				}
...
//		}
	...
	private void beforAuditUpdate(DrClaimsLoan drClaimsLoan,
		...
		if(Utils.strIsNull(drClaimsCustomer.getCompanyName())){
			...
			drClaimsCustomer.setLid(drClaimsLoan.getId());
			if(Utils.isObjectNotEmpty(queryClaimsCustomer)){
				drClaimsCustomer.setCompanyNameProtocolShow(drClaimsCustomer.getCompanyNameProtocolShow());
...
//						customer.setMchnt_txn_ssn(ssnCustomer.getMchnt_txn_ssn());
						...
						customer.setPhone(login_id);//这块的坑比,
		...
		try {
			...
			if(br.isSuccess()){
				...
				if("1".equals(user_st) && "0".equals(User_tp)){
					...
					customer.setUser_id(login_id);
					customer.setId(Integer.valueOf(param.get("id").toString()));
					customer.setPhone(login_id);   //customer.setPhone((String) result.get("mobile_no"));
					customer.setCertificateNo((String)result.get("certif_id"));
					customer.setCompanyName((String)result.get("cust_nm"));
					drClaimsCustomerDAO.updateDrClaimsCustomer(customer);
					//更新债权信息
...
//			drClaimsCustomer.setUpdataIdAndAcnt(1);
	...
	public DrClaimsLoan getDrClaimsLoanBySidB(Integer sid) {
		...
		return drClaimsLoanDAO.getDrClaimsLoanBySidB(sid);
	}
}
