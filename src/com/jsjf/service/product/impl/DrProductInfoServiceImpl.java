...
import com.jsjf.dao.product.JsCoverChargeDAO;
import com.jsjf.dao.product.JsInvoiceDAO;
...
public class DrProductInfoServiceImpl implements DrProductInfoService {
	...
	public BaseResult updateDrProductInfo(DrProductInfo drProductInfo,MultipartFile[] productFiles,@RequestParam MultipartFile acceptPicFile
		...
		if(Utils.isObjectEmpty(drProductInfo.getProductMarking())){
			drProductInfo.setChoose(100);
		}
...
//		List<DrProductInvest> investList = drProductInvestDAO.getDrProductInvestListByPid(info.getId());//查询未处理的投资记录
			...
			if(totalAmount.compareTo(info.getAmount()) < 0 
				...
				if(info.getRepayType() == 1){
					...
					DrProductInvestRepayInfo repayinfo = new DrProductInvestRepayInfo(invest.getUid(), invest.getId(), info.getId(),invest.getFactAmount(), BigDecimal.ZERO,
							...
							Utils.getDayNumOfAppointDate(now, -info.getDeadline()),null,null,null,null,null);
					
					insertRepayInfoList.add(repayinfo);
				}
				invest.setFactInterest(profitJson.getBigDecimal("interestProfit"));
				invest.setStatus(1);
				//协议编号 通过当日计息次数生成计息编号invest.setAgreementNo("JSJF"+Utils.format(new Date(), "yyMMdd")+info.getType()+new String(ary2));
				System.arraycopy((nums+"").toCharArray(), 0, ary2, ary2.length-(nums+"").toCharArray().length, (nums+"").toCharArray().length);
				invest.setAgreementNo("JSJF"+Utils.format(new Date(), "yyMMdd")+info.getType()+new String(ary2));
		...
		for (int j = 0; j < investList.size(); j++) {
			...
			if (totalAmount.compareTo(info.getAmount()) < 0 || info.getType() == 1 || info.getType() == 4
				...
				if(info.getRepayType() == 1){
					...
					DrProductInvestRepayInfo repayinfo = new DrProductInvestRepayInfo(invest.getUid(), invest.getId(), info.getId(),invest.getFactAmount(), BigDecimal.ZERO,
							profitJson.getBigDecimal("interestProfit"), BigDecimal.ZERO, BigDecimal.ZERO, 0,
							Utils.getDayNumOfAppointDate(now, -info.getDeadline()),profit,null,null,null,null);
				...
				invest.setStatus(1);
				// 协议编号
				// 通过当日计息次数生成计息编号invest.setAgreementNo("JSJF"+Utils.format(new
	...
	public JSONObject getInvestProfit(Integer deadline, DrProductInvest invest, BigDecimal dayRate, Integer productType,
		...
		BigDecimal interestRateCoupon = BigDecimal.ZERO;//加息券
		BigDecimal interestDoubleCoupons = BigDecimal.ZERO;//翻倍券
		...
		if (productType == 2 || productType == 6) {
			interestSubsidyDays=Integer.parseInt(Long.toString(Utils.getQuot(Utils.format(new Date(), "yyyy-MM-dd"),
		...
		if (invest.getFid() != null) {// 使用优惠券
			...
			if (dmf.getType() == 1) {// 红包
				...
				interestRateCoupon=invest.getFactAmount().multiply(activityRate)
						.multiply(new BigDecimal(day)).setScale(2, BigDecimal.ROUND_FLOOR);//加息券利息
				...
				interestDoubleCoupons=invest.getFactAmount().multiply(dRate).multiply(dmf.getMultiple().subtract(new BigDecimal(1)))
						.multiply(new BigDecimal(day).setScale(2, BigDecimal.ROUND_FLOOR));//翻倍券
		...
		obj.put("interestRateCoupon", interestRateCoupon);//加息券利息
		obj.put("interestDoubleCoupons", interestDoubleCoupons);//翻倍券
		return obj;
	}
			...
			if (productList.size() > 0) {
				for (DrProductInfo drproductInfo : productList) {
					...
					if (!Utils.isObjectEmpty(drproductInfo.getPrizeId())) {
						if(drProductInfoDAO.getProductPrize(drproductInfo.getPrizeId()) == 0){
...
//				BaseResult result= FuiouConfig.productUpdate(drProductInfo);
				...
				if (!flag) {
					try {
						SmsSendUtil.sendMsg("15801868241,15821421008,17317566419", drProductInfo.getFullName()+errorMsg);
					} catch (Exception e1) {
						e1.printStackTrace();
					}
				}
			...
			if (totalAmount.compareTo(info.getAmount()) < 0 || info.getType() == 1 || info.getType() == 4
				...
				if (invest.getStatus() == 0 /*|| invest.getStatus() == 4*/) {
					...
					if (info.getRepayType() == 1) {
						...
						BigDecimal profit = invest.getFactAmount().multiply(rate).multiply(new BigDecimal(info.getDeadline())).setScale(2,
								BigDecimal.ROUND_FLOOR);//基本利息
						
						BigDecimal platformInterest=invest.getFactAmount().multiply(dayRate.subtract(rate)).multiply(new BigDecimal(info.getDeadline())).setScale(2,
								BigDecimal.ROUND_FLOOR);//平台贴息
						
						BigDecimal interestRateCoupon = profitJson.getBigDecimal("interestRateCoupon");//加息券利息
						BigDecimal interestDoubleCoupons =profitJson.getBigDecimal("interestDoubleCoupons");//翻倍券利息
						BigDecimal interestSubsidy=profitJson.getBigDecimal("interestSubsidy");//未满标贴息
						
						BigDecimal interest=profit.add(platformInterest).add(interestRateCoupon).add(interestDoubleCoupons).add(interestSubsidy);
						if(interest.compareTo(profitJson.getBigDecimal("interestProfit"))==-1){//小于
							platformInterest=platformInterest.add(profitJson.getBigDecimal("interestProfit").subtract(interest));//若分开算的金额少于总的金额 则在平台贴息把差补全
						}
						if(interest.compareTo(profitJson.getBigDecimal("interestProfit"))==1){//大于
							platformInterest=platformInterest.subtract(interest.subtract(profitJson.getBigDecimal("interestProfit")));//若分开算的金额大于总的金额 则在平台贴息把差减去
						}						
						DrProductInvestRepayInfo repayinfo = new DrProductInvestRepayInfo(invest.getUid(),
								...
								profitJson.getBigDecimal("interestProfit"), BigDecimal.ZERO, BigDecimal.ZERO, 0,
								Utils.getDayNumOfAppointDate(now, -info.getDeadline()),profit,platformInterest,
								interestRateCoupon,interestDoubleCoupons,interestSubsidy);
						...
						BigDecimal interest = interestProfit.divide(new BigDecimal(periods),
								2, BigDecimal.ROUND_FLOOR);// 按总利息算每月利息
						
						BigDecimal basicprofit=profit.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);// 每月基本利息
						BigDecimal platform = platformInterest.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);//每月平台贴息
						BigDecimal rateCoupon=interestRateCoupon.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);//每月加息券利息
						BigDecimal doubleCoupons = interestDoubleCoupons.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);//每月翻倍券利息
						BigDecimal subsidy = interestSubsidy.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);//每月未满标贴息
						
						Date shouldTime = now;
						
						for (int i = 1; i <= periods; i++) {
							BigDecimal interestTemp = basicprofit.add(platform).add(rateCoupon).add(doubleCoupons).add(subsidy);
							BigDecimal basicprofitTemp=basicprofit;//每月基本利息
							BigDecimal platformTemp=platform;//每月平台贴息
							BigDecimal rateCouponTemp=rateCoupon;//每月加息券利息
							BigDecimal doubleCouponsTemp=doubleCoupons;//每月翻倍券利息
							BigDecimal subsidyTemp=subsidy;//每月未满标贴息
							BigDecimal interestcount=interest;//按总利息算每月利息
							if (i == periods) {
								//基本利息差
								BigDecimal basicprofitDiff = profit.subtract(basicprofitTemp.multiply(new BigDecimal(periods)));
								//平台贴息差
								BigDecimal platformDiff = platformInterest.subtract(platformTemp.multiply(new BigDecimal(periods)));
								//加息券利息差
								BigDecimal rateCouponDiff = interestRateCoupon.subtract(rateCouponTemp.multiply(new BigDecimal(periods)));
								//翻倍券利息差
								BigDecimal doubleCouponsDiff = interestDoubleCoupons.subtract(doubleCouponsTemp.multiply(new BigDecimal(periods)));
								//未满标贴息差
								BigDecimal subsidyDiff=interestSubsidy.subtract(subsidyTemp.multiply(new BigDecimal(periods)));
								//按总利息算每月利息差
								BigDecimal interestDiff=interestProfit.subtract(interestcount.multiply(new BigDecimal(periods)));

								// 差息放到最后一期
								interestTemp = interestTemp.add(basicprofitDiff).add(platformDiff).add(rateCouponDiff).add(doubleCouponsDiff).add(subsidyDiff);
								interestcount=interestcount.add(interestDiff);//按总利息算每月利息
								
								basicprofitTemp=basicprofitTemp.add(basicprofitDiff);//基本利息
								platformTemp=platformTemp.add(platformDiff);//平台贴息
								rateCouponTemp=rateCouponTemp.add(rateCouponDiff);//加息券利息
								doubleCouponsTemp=doubleCouponsTemp.add(doubleCouponsDiff);//翻倍券利息
								subsidyTemp=subsidyTemp.add(subsidyDiff);//未满标贴息
								
								shouldPrincipal = invest.getFactAmount();
							}
							
							if(interestcount.compareTo(interestTemp)==-1){//小于
								platformTemp=platformTemp.subtract(interestTemp.subtract(interestcount));//若分开算的金额大于总的金额 则在平台贴息把差减去
							}
							if(interestcount.compareTo(interestTemp)==1){//大于
								platformTemp=platformTemp.add(interestcount.subtract(interestTemp));//若分开算的金额小于总的金额 则在平台贴息把差补全
							}		
							shouldTime = Utils.getDayNumOfAppointDate(shouldTime, -30);
							DrProductInvestRepayInfo repayinfo = new DrProductInvestRepayInfo(invest.getUid(),
									invest.getId(), info.getId(), shouldPrincipal, BigDecimal.ZERO, interestcount,
									BigDecimal.ZERO, BigDecimal.ZERO, 0, shouldTime,basicprofitTemp,platformTemp,rateCouponTemp,doubleCouponsTemp,subsidyTemp);
							insertRepayInfoList.add(repayinfo);
						}
						...
						BigDecimal basicprofit=profit.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);// 每期基本利息
						BigDecimal platform = platformInterest.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);//每期平台贴息
						BigDecimal rateCoupon=interestRateCoupon.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);//每期加息券利息
						BigDecimal doubleCoupons = interestDoubleCoupons.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);//每期翻倍券利息
						BigDecimal subsidy = interestSubsidy.divide(new BigDecimal(periods), 2,
								BigDecimal.ROUND_FLOOR);//每期未满标贴息
						...
						for (int i = 1; i <= periods; i++) {
							...
							BigDecimal interestTemp = basicprofit.add(platform).add(rateCoupon).add(doubleCoupons).add(subsidy);//总利息
							BigDecimal basicprofitTemp=basicprofit;//每月基本利息
							BigDecimal platformTemp=platform;//每月平台贴息
							BigDecimal rateCouponTemp=rateCoupon;//每月加息券利息
							BigDecimal doubleCouponsTemp=doubleCoupons;//每月翻倍券利息
							...
							DrProductInvestRepayInfo repayinfo = new DrProductInvestRepayInfo(invest.getUid(),
									invest.getId(), info.getId(), factprincipal, BigDecimal.ZERO, interestcount,
									BigDecimal.ZERO, BigDecimal.ZERO, 0, shouldTime,basicprofitTemp,platformTemp,rateCouponTemp,doubleCouponsTemp,subsidyTemp);
							insertRepayInfoList.add(repayinfo);
						}
					...
					invest.setAgreementNo(
							"JSJF" + Utils.format(new Date(), "yyMMdd") + info.getType() + new String(ary2));
	...
	public BaseResult updateDrProductLoanByMchntTxnSsn(String mchnt_txn_ssn)
		...
		if(drProductInfo!=null){
			...
			accountLog.setCompanyfunds(79);//资金类型 放款失败
			accountLog.setType(1);//收入
			accountLog.setAmount(drProductInfo.getAmount());//金额
			/*accountLog.setBalance(jsCompanyAccountLogDAO.getBlanceByFuiou().subtract(new BigDecimal(3)));*/
			accountLog.setStatus(3);//成功
			accountLog.setRemark(drProductInfo.getCode()+"产品放款失败");
	...
	public BaseResult queryTransferBmu() {
		...
		for (int i = 0; i < investRepayInfoList.size(); i++) {
			...
			try {
				...
				othreInterestMap.put("type", repayInfo.getProductType());//产品类型
	...
	public void productFullfreezeToFreeze(Map<String,Object> map) {
		...
		if(Utils.isObjectNotEmpty(info) && (info.getStatus() ==6 || info.getStatus() ==8)){	
			...
			if(!Utils.isEmptyList(updateList)){
				...
				} catch (Exception e) {//一般这里报错都是锁表造成的			
					e.printStackTrace();
					try {
						Thread.sleep(5000l);
						//
			...
			if((investList.size() >0 && failNum >0) || investList.size()==0 ){//有失败的,param 保存到redis 里,并发短信
				...
				try {
					SmsSendUtil.sendMsg("15618538752,17317566419", "产品满标-冻结到冻结["+info.getFullName()+","+info.getId()+"] 投资数["+investList.size()+"] 失败数["+failNum+"]");
				} catch (Exception e) {
					System.out.println("产品满标-冻结到冻结-发短信失败");
					e.printStackTrace();
				}
				//		
				...
				companyAccountLog.setCompanyfunds(69);//产品满标
				companyAccountLog.setType(1);//收入
				companyAccountLog.setAmount(info.getAmount());//金额
				companyAccountLog.setStatus(3);//成功
				companyAccountLog.setRemark(info.getCode()+"产品满标");
				companyAccountLog.setAddTime(new Date());
				companyAccountLog.setChannelType(2);//存管
				companyAccountLog.setPid(info.getId());
...
//					System.out.println(smsList.get(i).getMessage());
			...
			if(syslog.getIcd().equals("000015") && syslog.getProject_no()!=null && syslog.getStatus()!=2){//订单是企业网银充值并且log日志状态未成功
				...
				if(br.isSuccess()){
					if(br.getMsg().contains("0000|成功")){
			    		...
			    		JsCompanyAccountLog accountLog=new JsCompanyAccountLog();
			    		accountLog.setCompanyfunds(66);//资金类型 企业还款
			    		accountLog.setType(1);//收入
			    		accountLog.setAmount(amount);//金额
			    		accountLog.setStatus(3);//成功
			    		accountLog.setAddTime(new Date());
			    		accountLog.setChannelType(2);//存管
			    		accountLog.setPid(drProductInfo.getId());
						if(drProductInfo.getRepayType() == 2 || drProductInfo.getRepayType() == 3 || drProductInfo.getRepayType() == 4){//等本等息，按月付息产品
			    			Map<String, Object> ma=new HashMap<>();
			    			
			    			
			    			ma.put("id", syslog.getPdId());//期数明细的ID
		    				List<DrProductInfoRepayDetail> li = drProductInfoRepayDetailDAO.getrepayDetailById(ma);

		    				ma.clear();
		    				ma.put("id", syslog.getPdId());//期数明细的ID
		    				ma.put("loanStatus", 2);//已回款
		    				drProductInfoRepayDetailDAO.updateLoanStatusById(ma);
		    				
		    				ma.clear();
		    				ma.put("pid", drProductInfo.getId());
		    				List<DrProductInfoRepayDetail> list = drProductInfoRepayDetailDAO.getrepayDetailById(map);
			    			if(list.size()==0){//已还清
			    				drProductInfoDAO.updateReFundDrProductLoanStatus(drProductInfo.getId());//已还清
			    			}	
			    			
			    	
			    			DrCompanyFundsLog drCompanyFundsLog = new DrCompanyFundsLog(16, null, 
				    				drProductInfo.getId(), amount, 1, 
				    						"债券合同["+loanNo+"]"+li.get(0).getPeriods()+"回款"+amount+"元,流水号["+tran_flowid+"]",null);
				    		drCompanyFundsLogService.insertDrCompanyFundsLog(drCompanyFundsLog);
				    		
				    		accountLog.setRemark("债券合同["+loanNo+"]"+li.get(0).getPeriods()+"回款"+amount+"元");
			    		}else{
				    		...
				    		accountLog.setRemark("债券合同["+loanNo+"]回款"+amount+"元");
			    		}
						...
						JsCompanyAccountLog companyAccountLog=new JsCompanyAccountLog();
						companyAccountLog.setCompanyfunds(58);//资金类型
	...
	@Override
	public BaseResult getInstallmentRepaymentProductInfo(Integer id,PageInfo pi) {
		Map<String, PageInfo> resultMap = new HashMap<String, PageInfo>();
		List<Map<String, Object>> list = drProductInfoDAO.getInstallmentRepaymentProductInfo(id);
		pi.setRows(list);
		resultMap.put("page", pi);
		BaseResult br = new BaseResult();
		br.setMap(resultMap);
	...
	public Map<String, Object> getDepositReturnLoanListSum(DrProductInfo drProductInfo) {
		...
		param = getExtractMap(drProductInfo);
		param.put("loginId", drProductInfo.getLoginId());
		return drProductInfoDAO.getDepositReturnLoanListSum(param);
	}

	@Override
	public BaseResult cgReturnedMoney(DrProductInfo drProductInfo, String phone, BigDecimal alreadyRaiseAmount, 
			DrSubjectInfo drSubjectInfo, String tran_flowid , String actLoanTime, SysUsersVo usersVo, DrCompanyFundsLog drCompanyFundsLog) {
		...
		if(drProductInfo.getProject_no()!=null){
			try {
				...
				//记公司账户日志 收取手续费
				JsCompanyAccountLog accountLog=new JsCompanyAccountLog();
				accountLog.setCompanyfunds(75);//资金类型 放款成功
				accountLog.setType(0);//支出
				accountLog.setAmount(drProductInfo.getAmount());//金额
				/*accountLog.setBalance(jsCompanyAccountLogDAO.getBlanceByFuiou().subtract(new BigDecimal(3)));*/
				accountLog.setStatus(3);//成功
				accountLog.setRemark(drProductInfo.getCode()+"产品放款");
				accountLog.setAddTime(new Date());
				accountLog.setChannelType(2);//存管
				accountLog.setPid(drProductInfo.getId());
				jsCompanyAccountLogDAO.insertCompanyAccountLog(accountLog);	
				...
				br.setMsg("放款成功");
				br.setSuccess(true);
				updateLoanStatus(drProductInfo.getId(),usersVo.getUserKy().intValue(),actLoanTime,tran_flowid);
